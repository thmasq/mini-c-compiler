...
type_info_t merge_declaration_specifiers(type_info_t base, declarator_t declarator)
{
	...
	result.is_vla = declarator.is_array && declarator.array_size && declarator.array_size->type != AST_NUMBER;
...
int is_integer_type(type_info_t *type)
{
	...
	return (strcmp(type->base_type, "char") == 0 || strcmp(type->base_type, "short") == 0 ||
		strcmp(type->base_type, "int") == 0 || strcmp(type->base_type, "long") == 0 ||
		strcmp(type->base_type, "_Bool") == 0 || strstr(type->base_type, "signed") != NULL ||
		strstr(type->base_type, "unsigned") != NULL || type->is_enum);
}
...
int is_floating_type(type_info_t *type)
{
	...
	return (strcmp(type->base_type, "float") == 0 || strcmp(type->base_type, "double") == 0);
}
...
int can_convert_to(type_info_t *from, type_info_t *to)
{
	...
	// Identical types
	if (from->pointer_level == to->pointer_level && from->is_array == to->is_array &&
...
static void traverse_array_declaration(ast_node_t *node, symbol_table_t *table)
{
	...
	if (add_symbol(table, node->data.array_decl.name, SYM_VARIABLE, node->data.array_decl.type_info) == NULL) {
		error_count++;
	}
}
...
static void traverse_struct_decl(ast_node_t *node, symbol_table_t *table)
{
	...
	if (node->data.struct_decl.is_definition) {
		type_info_t struct_type = create_type_info(string_duplicate(node->data.struct_decl.name), 0, 0, NULL);
		struct_type.is_struct = 1;

		symbol_t *struct_sym = add_symbol(table, node->data.struct_decl.name, SYM_STRUCT, struct_type);

		if (struct_sym) {
			member_info_t *member = node->data.struct_decl.members;
			while (member) {
				symbol_t *member_sym = create_symbol(member->name, SYM_VARIABLE, member->type);
...
static void traverse_union_decl(ast_node_t *node, symbol_table_t *table)
{
	...
	if (node->data.union_decl.is_definition) {
		type_info_t union_type = create_type_info(string_duplicate(node->data.union_decl.name), 0, 0, NULL);
		union_type.is_union = 1;

		symbol_t *union_sym = add_symbol(table, node->data.union_decl.name, SYM_UNION, union_type);
...
static void traverse_enum_decl(ast_node_t *node, symbol_table_t *table)
{
	...
	if (node->data.enum_decl.is_definition) {
		type_info_t enum_type = create_type_info(string_duplicate(node->data.enum_decl.name), 0, 0, NULL);
		enum_type.is_enum = 1;
...
static void traverse_typedef(ast_node_t *node, symbol_table_t *table)
{
	...
	if (add_symbol(table, node->data.typedef_decl.name, SYM_TYPEDEF, node->data.typedef_decl.type) == NULL) {
		error_count++;
	}
}
...
static void print_type_info(type_info_t t)
{
	...
	if (t.pointer_level > 0)
		print_ptr_stars(t.pointer_level);
}
...
void print_ast(ast_node_t *node, int indent)
{
	...
	case AST_FOR_STMT:
		...
		if (node->data.for_stmt.condition) {
			indent_out(indent + 2);
			puts("cond:");
			print_ast(node->data.for_stmt.condition, indent + 4);
		}
		if (node->data.for_stmt.update) {
			indent_out(indent + 2);
			puts("upd:");
			print_ast(node->data.for_stmt.update, indent + 4);
		}
