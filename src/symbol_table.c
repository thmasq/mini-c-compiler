...
char *generate_unique_name(symbol_table_t *table, const char *base_name)
{
	...
	if (table->current_function) {
		snprintf(unique_name, 256, "%s.%s.%d.%d", table->current_function, base_name,
...
void add_struct_member(symbol_t *struct_sym, symbol_t *member)
{
	...
	struct_sym->members = realloc(struct_sym->members, struct_sym->member_count * sizeof(symbol_t *));
...
void print_symbol(symbol_t *sym, int indent)
{
	...
	case SYM_STRUCT:
		printf("struct, size=%zu, align=%zu, members=%d", sym->total_size, sym->max_alignment,
		       sym->member_count);
		break;

	case SYM_UNION:
		printf("union, size=%zu, align=%zu, members=%d", sym->total_size, sym->max_alignment,
		       sym->member_count);
		break;
