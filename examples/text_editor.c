...
int editorReadKey() {
    ...
    if (c == 27) {
        if (read(0, seq, 1) == 0) return 27;
        if (read(0, seq + 1, 1) == 0) return 27;

        if (seq[0] == '[') {
            if (seq[1] >= '0' && seq[1] <= '9') {
                if (read(0, seq + 2, 1) == 0) return 27;
                if (seq[2] == '~') {
...
void editorMoveCursor(int key) {
    ...
    if (key == KEY_ARROW_LEFT) {
        ...
        if (E_cx < E_screencols - 1) E_cx++;
    } else if (key == KEY_ARROW_UP) {
        if (E_cy != 0) E_cy--;
    } else if (key == KEY_ARROW_DOWN) {
        if (E_cy < E_numrows) E_cy++;
    }
...
void editorProcessKeypress() {
    ...
    if (c == KEY_ARROW_UP || c == KEY_ARROW_DOWN || 
        c == KEY_ARROW_LEFT || c == KEY_ARROW_RIGHT) {
        editorMoveCursor(c);
        return;
    }
}
    ...
    for (y = 0; y < E_screenrows; y++) {
        ...
        if (filerow >= E_numrows) {
            if (E_numrows == 0 && y == E_screenrows / 3) {
                ...
                if (welcomelen > E_screencols) welcomelen = E_screencols;
            ...
            if (len < 0) len = 0;
            if (len > E_screencols) len = E_screencols;
